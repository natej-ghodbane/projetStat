...
\addcontentsline{toc}{part}{PROJECT DOCUMENTATION}
...
The objective of this study is to identify dietary patterns among Chinese women and examine their associations with blood lipid biomarkers, including high-density lipoprotein cholesterol (HDL-C), low-density lipoprotein cholesterol (LDL-C), triglycerides (TG), and total cholesterol (TC).

Other biomarkers available in the dataset may be indirectly associated with lipid metabolism (such as glucose or insulin); however, they are not considered direct components of the blood lipid profile and were therefore not included in this analysis.

Only data from the 2009 survey wave were included in the analysis, as both dietary intake and blood biomarker measurements were collected during this wave. This restriction ensures temporal consistency between dietary exposure and lipid outcomes and aligns with the methodology of the reference study.
...
The factor scores derived from the Principal Component Analysis (PCA) were incorporated into the dataset to represent each participant’s degree of adherence to the identified dietary patterns.  
These continuous scores were subsequently categorized into **quartiles**, allowing comparison of **blood lipid biomarkers** across increasing levels of adherence to each dietary pattern (from the lowest to the highest quartile).  
This classification improves interpretability and enables the application of **comparative statistical tests**, such as ANOVA and the Kruskal–Wallis test.
...
table(data$Pattern2_Q)
table(data$Pattern3_Q)
```
...
One-way ANOVA revealed significant differences in total cholesterol (TC) and triglycerides (TG) across quartiles of all three dietary patterns (p < 0.01), indicating that lipid levels vary according to the degree of adherence to each pattern.
For LDL-C, significant differences were observed across quartiles of Pattern1 and Pattern2, while no significant variation was found for Pattern3.
In contrast, HDL-C showed a significant difference only across quartiles of Pattern1, with no significant association for Pattern2 or Pattern3
...
Kruskal–Wallis tests corroborated the ANOVA findings, showing significant associations of dietary pattern quartiles with TC and TG, partial associations with LDL-C, and limited associations with HDL-C.
...
spearman_results <- rbind(
  # Fat
  extract_spearman(data$TG,    data$d3fat,    "TG",    "Fat"),
  extract_spearman(data$TC,    data$d3fat,    "TC",    "Fat"),
  extract_spearman(data$LDL_C, data$d3fat,    "LDL-C", "Fat"),
  extract_spearman(data$HDL_C, data$d3fat,    "HDL-C", "Fat"),
  
  # Carbohydrates
  extract_spearman(data$TG,    data$d3carbo,  "TG",    "Carbohydrates"),
  extract_spearman(data$TC,    data$d3carbo,  "TC",    "Carbohydrates"),
  extract_spearman(data$LDL_C, data$d3carbo,  "LDL-C", "Carbohydrates"),
  extract_spearman(data$HDL_C, data$d3carbo,  "HDL-C", "Carbohydrates"),
  
  # Protein
  extract_spearman(data$TG,    data$d3protn,  "TG",    "Protein"),
  extract_spearman(data$TC,    data$d3protn,  "TC",    "Protein"),
  extract_spearman(data$LDL_C, data$d3protn,  "LDL-C", "Protein"),
  extract_spearman(data$HDL_C, data$d3protn,  "HDL-C", "Protein")
)
...
Spearman correlation analyses showed statistically significant but weak associations between macronutrient intake and blood lipid biomarkers.
Fat intake was positively correlated with TG, TC, and LDL-C, while no significant association was observed with HDL-C.
Carbohydrate intake showed weak negative correlations with TC, LDL-C, and HDL-C, and no significant association with TG.
Protein intake was weakly positively associated with TG and TC, whereas associations with LDL-C were not significant and weak negative correlations were observed with HDL-C.
---
